            ...
            this.setState({
                ...
                axios.get(`http://185.157.246.81:5000/search/${questionValue}`)
                    .then(request => {
                        ...
                        // data = [
                        //     {id:39642, title:"Test", format:"html", keywords:["none"]}, 
                        //     {id:0, title:"Two Planes three towers", format:"PDF", keywords:["tower", "plane", "jew", "luck"]}, 
                        //     {id:0, title:"Wigou Vigoula", format:"PDF", keywords:["gitan", "wesh", "derulo", "professionnel"]},
                        //     {id:0, title:"Random book", format:"PDF", keywords:["random"]},
                        //     {id:0, title:"Random book", format:"Random Author", keywords:["random"]},
                        //     {id:0, title:"Random book", format:"Random Author", keywords:["random"]},
                        //     {id:0, title:"Random book", format:"Random Author", keywords:["random"]},
                        //     {id:0, title:"Random book", format:"Random Author And Me", keywords:["random"]},
                        //     {id:0, title:"Random book", format:"Random Author", keywords:["random"]},
                        // ];
                        this.setState({
                            waitingData: false,
        ...
        return (
            <Fragment>
                ...
                <PopoverList className={this.state.isAnswering ? "answer" : ""} id={"scene-knowledge-block-character-popover"} placement={this.state.clickedCharacter ? "top" : "left"} isOpen={(!this.props.dismissPopover && this.props.visible && this.state.hoveringCharacter)} target={"scene-knowledge-block-character"}
                    title={"Librarian"} content={this.state.data.map((item, i) => <li key={i}><a href="#" onClick={() => { this.props.secondKnowledgePanel(item.id) }}><span className={"list-document-title"}>{item.title}</span><span className={"list-document-author"}>{item.format}</span><span className={"list-document-keywords"}>{item.keywords.join(" ")}</span></a></li>)}
