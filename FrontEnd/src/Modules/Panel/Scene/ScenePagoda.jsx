...
export default class ScenePagoda extends Component {
    ...
    componentDidMount = () => {
        ...
        let data = [];
        axios.get(`http://185.157.246.81:5000/prerequisites/${this.props.idDocument}`)
        .then(request => {
            let datas = request.data;

            let dataper2 = Math.ceil(datas.length / 2);
            for (let i = 0; i < dataper2; i++) {
                let temp_array = [];
                if (i === dataper2 - 1 && datas.length % 2 === 1) {
                    temp_array.push({ title: datas[2 * i][0], link: datas[2 * i][1] });
                } else {
                    temp_array.push({ title: datas[2 * i][0], link: datas[2 * i][1] });
                    temp_array.push({ title: datas[2 * i + 1][0], link: datas[2 * i + 1][1]});
                }
